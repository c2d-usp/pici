...
class InitScalable:
    def calculateEmpiricals(M: int, N: int, df, DBG = False):
        ...
        for realizationCase in list(product([0, 1], repeat= N + M + 1)):
            currentProbability: float = find_probability2(dataFrame=df, realizationDict={ "X": realizationCase[0]})

            targetRealization: dict[str, int] = {}; conditionRealization: dict[str, int] = {}
            ...
            for ShiftedBindex in range(N + 1, N + M + 1):                
                conditionRealization[f"B{ShiftedBindex - N}"] = realizationCase[ShiftedBindex]

            currentProbability *= find_conditional_probability2(dataFrame=df, 
    ...
    def defineGammaUAuxiliaryVariables(M: int, N: int, df, XValue: int, targetValue: int, DBG = False):
        betaVarsBits: list[tuple[list[str]]] = [0] * (1 << (N + M)) # For each: (B+, B-), with B+ = list[str]
        betaVarsCoeffObjSubproblem: list[float] = [0] * (1 << (N + M))

        bitPlus: list[int] = []; bitMinus: list[int] = []

        # Order of realization: (A1,A2,..,An,B1,B2,...,Bm)
        for betaVarIndex, realizationCase in enumerate(list(product([0, 1], repeat= M + N))):            
            # Pre-compute B contribution:
            bitIndexLower = 0; indexBs = N + M - 1; helper = 1
            while (indexBs >= N) :
            ...
            for clusterBitIndex in range(1, N + 1):
                ...
                if realizationCase[clusterBitIndex - 1] == 1:
                    bitPlus.append(f"A{clusterBitIndex}_{bitIndex}") # First the mechanism and then the index in the mechanism bit cluster
                else:
                    bitMinus.append(f"A{clusterBitIndex}_{bitIndex}")

            # ----- Calculate the obj coeff:
            objCoeff: float = 1.0
            objCoeff *= find_conditional_probability2(dataFrame=df, targetRealization= { "Y": targetValue },
            ...
            objCoeff *= find_conditional_probability2(dataFrame=df, targetRealization = targetRealization,
    ...
    def defineParametricColumn(M: int, N: int):
        ...
        for restrictionIndex in range((1 << N + M + 1)):
            if (restrictionIndex < (1 << M + N)): xRealization = 0 
            else :                                xRealization = 1
...
def testBetaVars():
    ...
    for realizationCase in list(product([0, 1], repeat=4)):
        prob = find_conditional_probability2(dataFrame=df,
