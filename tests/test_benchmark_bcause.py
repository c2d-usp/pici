...
class TestBenchmarkBcauseInference(unittest.TestCase):
    def test_bcause_1(self):
        ...
        unobs = ["U"]
        df = pd.read_csv(
            os.path.join(PROJECT_ROOT, DataExamplesPaths.CSV_BCAUSE_EXAMPLE_1.value)
        )
        bcause = {
            0: [0.12651440458851518, 0.41206478674401825],
        ...
        for intervention_value in [0, 1]:
            with self.subTest(intervention_value=intervention_value):
                ...
                model = CausalModel(data=df, edges=edges, unobservables_labels=unobs)

                lower, upper = model.intervention_query(
                    interventions=[("X", intervention_value)], target=("Y", 1)
                )
    ...
    def test_bcause_2(self):
        ...
        unobs = ["U", "U2"]
        df = pd.read_csv(
            os.path.join(PROJECT_ROOT, DataExamplesPaths.CSV_BCAUSE_EXAMPLE_2.value)
        )
        df["U2"] = np.random.binomial(1, 0.5, size=len(df))
        bcause = {
            0: [0.3991619515659455, 0.39916195179153174],
        ...
        for intervention_value in [0, 1]:
            with self.subTest(intervention_value=intervention_value):
                ...
                model_2 = CausalModel(data=df, edges=edges, unobservables_labels=unobs)
