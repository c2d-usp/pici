...
from pici.intervention_inference_algorithm.column_generation.generic.bits import generate_optimization_problem_bit_list
...
class TestBitsCalculation(unittest.TestCase):
    def test_iv_binary_correct(self):
        ...
        self.assertEqual(int(len(bits_list)), 4, msg=f"Values do not match for case 'Z -> X, X -> Y, U1 -> X, U1 -> Y'",)
    ...
    def test_intervention_without_endogenous_parent_correct(self):
        ...
        bits_list = generate_optimization_problem_bit_list(X)
        self.assertEqual(int(len(bits_list)), 7, msg=f"Values do not match for case 'Z -> K, W -> K, X -> Y, U1 -> K, U1 -> X'")
