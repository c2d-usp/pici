...
class Gamma:
    ...
    def get_coef_from_objective_function(self, w_header: list, w_realization: list):
        ...
        for w_conditional_probability in self.Pw:
            ...
            coefw *= find_conditional_probability(
                dataFrame=self.df,
        ...
        for q_realization in q_realizations:
            ...
            for q_conditional_probability in self.Pq:
                ...
                coef_parcial *= find_conditional_probability(
                    dataFrame=self.df,
    ...
    def linearize(
        ...
        considered_c_component_in_topological_order: list[Node],
    ) -> dict:
        """
        ...
        for realization in cartesian_products:
            ...
            for node in considered_c_component_in_topological_order:
                ...
                parents_realization = [
                    realization[header.index(parent_label)]
                    for parent_label in parents_label
                ]
                realization_key: str = self.get_realization_key(
                    parents_label, parents_realization
                )
            ...
            gamma_u_map_bit_product_to_linearized_variable[bit_product] = (
                self.model.addVar(obj=coef, vtype=GRB.BINARY)
            )
    ...
    def _get_node_bit_variable_given_parents_realization(
    ...
    def add_linearized_bit_products_constraints(
        ...
        for (
            bit_product,
            variable,
        ) in gamma_u_map_bit_product_to_linearized_variable.items():
