...
class SubProblem:
    ...
    def setup(
        ...
        self.get_A_u_column_and_parameterized_column(reversed_ordered_W_realizations, reversed_ordered_considered_c_comp)
        self.generate_linearized_bit_products_constraints(self.a_u_map_bit_product_to_linearized_variable, name="au")
        self.model.update()
    ...
    def _create_cluster_bits(self, considered_c_comp: list[Node]):
        ...
        for node in considered_c_comp:
            ...
            for i, realization in enumerate(reversed_ordered_node_parents_realizations):
                ...
                logger.debug(f"    {j}th - Node {node.label} Realization key: {realization_key}--{realization} ==> {node_number_of_bits} bits")
                j+=1
    ...
    def get_coef_from_objective_function(self, w_header: list, w_realization: list):
        ...
        for w_conditional_probability in self.Pw:
            ...
            logger.debug(f"P({w_target.label}={w_target.value}|{str_w[:len(str_w)-2]}) == {curr}")
        ...
        for q_realization in q_realizations:
            ...
            for q_conditional_probability in self.Pq:
                ...
                coef_parcial *= curr
                logger.debug(f"P({q_target.label}={q_target.value}|{str_q[:len(str_q)-2]}) == {curr}")
    ...
    def get_A_u_column_and_parameterized_column(self, total_w_realization, considered_c_component_in_topological_order):
        ...
        for i, realization in enumerate(total_w_realization):
            ...
            self.a_u_map_bit_product_to_linearized_variable[bit_product] = self.model.addVar(vtype=GRB.BINARY)
            self.parameterized_column[i] = self.a_u_map_bit_product_to_linearized_variable[bit_product]
    ...
    def generate_bit_product(self, node_list: list[Node], header: list[str], realization: list[int], consider_intervention=False):
            ...
            for node in node_list:
                ...
                if len(binary_node_value) > node_number_of_bits:
                    ...
                    binary_node_value = (node_number_of_bits-len(binary_node_value))*"0" + binary_node_value

                for variable_index in range(node_number_of_bits):
                    sign = 1
                    if int(binary_node_value[variable_index]) == 0:
                        sign = -1
